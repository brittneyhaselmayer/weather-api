...
class Api::V1::WeathersController < ApplicationController
    ...
    def find_weather(type, lat, lon, unit)
        ...
        when "\"hourly\""
            request_api("https://api.weatherbit.io/v2.0/forecast/hourly?lat=#{lat}&lon=#{lon}&units=#{unit}&key=#{ENV["WEATHER_API_KEY"]}")
        when "\"daily\""
            request_api("https://api.weatherbit.io/v2.0/forecast/daily?lat=#{lat}&lon=#{lon}&days=5&units=#{unit}&key=#{ENV["WEATHER_API_KEY"]}")
        else
            request_api("https://api.weatherbit.io/v2.0/current?lat=#{lat}&lon=#{lon}&units=#{unit}&key=#{ENV["WEATHER_API_KEY"]}")
        end
    end 
